...
int GB_InsertKeyword(LIST* l, int code){
	...
	int count = 0;
	int k = 0;
	while (n->site->keyw[count][0] != NULL || count > 10){
		count++;
	}
	if (count > 10){
...
int GB_InsertSite(LIST* l, int pos){	          
	if (GB_CheckInvalidList(l)) return ERROR; 
	GB_GetSiteStdin(l);  /*the function that updates all the site's infos is called to the new node*/
					/*then we insert the new node at the given position 'pos'*/
	return SUCCESS;
}
